...
# This allows importing xml_quote_generator when running as a subprocess
...
async def list_tools() -> List[Tool]:
    ...
    logger.info("=" * 60)
    logger.info(f"📋 [RequestID: {request_id}] list_tools() called - preparing tool definitions")
    try:
        tools = [
        Tool(
        ...
        logger.info(f"✅ [RequestID: {request_id}] Returning {len(tools)} tool(s): {', '.join(tool_names)}")
        logger.info("=" * 60)
        return tools
    except Exception as e:
        logger.error(f"❌ [RequestID: {request_id}] Error in list_tools(): {str(e)}", exc_info=True)
        logger.info("=" * 60)
        raise
...
async def call_tool(name: str, arguments: Dict[str, Any]) -> List[TextContent]:
    ...
    logger.info("=" * 60)
    logger.info(f"🔧 [RequestID: {request_id}] MCP Tool Call: {name}")
    logger.info(f"📥 [RequestID: {request_id}] Arguments: {json.dumps(arguments, indent=2, ensure_ascii=False)}")
    logger.info(f"⏱️  [RequestID: {request_id}] Started at {time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(start_time))}")
    logger.info("=" * 60)
    ...
    try:
        if name == "generate_quote_for_products":
            ...
            logger.info(f"🔍 [RequestID: {request_id}] Searching for query: '{query}' in collection: '{collection_name}' (limit: {limit})")
            ...
            try:
                ...
                async with httpx.AsyncClient(timeout=30.0) as client:
                    ...
                    if response.status_code == 200:
                        ...
                        if content:
                            ...
                            elapsed_time = time.time() - start_time
                            logger.info(f"✅ [RequestID: {request_id}] Tool 'file_search' completed (no results) in {elapsed_time:.2f}s")
                            logger.info("=" * 60)
                            return [TextContent(type="text", text=f"No relevant content found for query: '{query}'")]
                    else:
                        error_detail = response.json().get('detail', 'Unknown error') if response.headers.get('content-type', '').startswith('application/json') else response.text
                        logger.error(f"❌ [RequestID: {request_id}] RAG service returned error: {response.status_code} - {error_detail}")
                        elapsed_time = time.time() - start_time
                ...
                elapsed_time = time.time() - start_time
                logger.error(f"❌ [RequestID: {request_id}] Search request timed out after 30 seconds for query: '{query}'")
                logger.info(f"⏱️  [RequestID: {request_id}] Tool 'file_search' timed out after {elapsed_time:.2f}s")
                logger.info("=" * 60)
                return [TextContent(type="text", text="Search request timed out. Please try again.")]
            except httpx.ConnectError as e:
                elapsed_time = time.time() - start_time
                logger.error(f"❌ [RequestID: {request_id}] Could not connect to RAG service at {RAG_SERVICE_URL}: {e}")
                logger.info(f"⏱️  [RequestID: {request_id}] Tool 'file_search' failed after {elapsed_time:.2f}s")
                logger.info("=" * 60)
            ...
            try:
                ...
                elapsed_time = time.time() - start_time
                logger.error(f"[RequestID: {request_id}] Glosil product details request timed out for Product ID: {product_id}")
            ...
            logger.warning(f"❌ [RequestID: {request_id}] Unknown tool requested: {name}")
            elapsed_time = time.time() - start_time
            logger.info(f"⏱️  [RequestID: {request_id}] Tool '{name}' completed in {elapsed_time:.2f}s (UNKNOWN TOOL)")
            logger.info("=" * 60)
            return [TextContent(type="text", text=f"Unknown tool: {name}")]
    
    except Exception as e:
        elapsed_time = time.time() - start_time
        logger.error(f"❌ [RequestID: {request_id}] Unexpected error in call_tool for {name}: {str(e)}", exc_info=True)
        logger.info(f"⏱️  [RequestID: {request_id}] Tool '{name}' failed after {elapsed_time:.2f}s")
        logger.info("=" * 60)
