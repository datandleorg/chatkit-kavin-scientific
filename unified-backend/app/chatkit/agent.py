...
Uses function tools directly (no MCP server subprocess)
"""
...
def create_agent(model: str = "gpt-5") -> Agent:
    """Create and return an Agent instance with all tools"""
    logger.info("=" * 60)
    logger.info("🤖 Creating Agent with Function Tools")
    logger.info(f"  📋 Tools: generate_quote_for_products, file_search, search_hyma, get_hyma_product_details, search_spectrochem, get_spectrochem_product_details, search_glosil, get_glosil_product_details, search_tci, get_tci_product_details")
    logger.info("  🧠 Thinking tokens: Enabled")
    logger.info("=" * 60)
    
    # Note: Thinking tokens are only supported by o1, o3 models
    # To enable thinking tokens, use: model="o1-preview" or model="o3-mini"
    # gpt-5 does NOT support thinking tokens
    logger.info(f"  🤖 Model: {model}")
    if model in ["o1-preview", "o1", "o3-mini", "o3"]:
        logger.info("  ✅ Thinking tokens: Supported by this model")
    else:
        logger.info("  ⚠️  Thinking tokens: NOT supported by this model (use o1 or o3 for thinking tokens)")
    ...
    return Agent(
        ...
        model_settings=ModelSettings(
            store=True,
            # Enable thinking/reasoning tokens if supported by the model
            # Note: Only o1 and o3 models support thinking tokens
        ),
        tools=[
